...
sy3_error SynexensROSDevice::renderRgbToROS(sensor_msgs::ImagePtr& rgb_image, sy3::frame* sy3_rgb_frame)
{
  ...
  rgb_image = cv_bridge::CvImage(std_msgs::Header(), sensor_msgs::image_encodings::TYPE_8UC3, rgb_buffer_mat).toImageMsg();
...
{
  sy3::sy3_error e;
  sy3::process_engine* engine = sy3_pline_->get_process_engin(e);
	sy3::points* points = engine->comptute_points(depth_image, e);
